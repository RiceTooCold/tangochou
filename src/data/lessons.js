...
export const LESSONS = [l7, l8, l9, l10, l11, l12]

export const LESSON_NUMBERS = LESSONS.map((l) => l.lesson)

// Default focus for practice + list filters (the lessons being studied right now).
export const DEFAULT_LESSONS = [10, 11, 12]

// Flat list of all words across all lessons (ids are globally unique, e.g. "l9-001").
export const ALL_WORDS = LESSONS.flatMap((l) => l.words)

const WORDS_BY_LESSON = new Map(LESSONS.map((l) => [l.lesson, l.words]))
...
export function wordsForLessons(lessons) {
...
export function sectionsOf(words) {
  const seen = []
  const set = new Set()
